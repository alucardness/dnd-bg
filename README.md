...
<ol>
  ...
  <br>
  <p>Ако общият резултат е равен или по-голям от желаното число, <code>ability check</code>, <code>attack rolls</code> или <code>saving throws</code> е успешно. В противен случай е провал. Обикновено DM на играта определя целевите числа и казва на играчите дали техните проверки са успешни или не.</p>

  <p>Желаното число за проверка на <code>ability</code> или <code>saving throw</code> се нарича <code>Difficulty Class (DC)</code>. Желаното число за <code>attack rolls</code> се нарича <code>Armor Class (AC)</code>.</p>
...
<br>
<h2>Странични правила при хвърляне</h2>

<ul>
  <li>Предимство и Недостатък

  <br>
  <p>Понякога проверка на <code>ability</code>, <code>attack rolls</code> или <code>saving throw</code> се модифицира от специални обстоятелства, наречени <coded>предимство и <code>недостатък</code>. Предимството отразява положителните обстоятелства около хвърлянето на 20-стенния зар, докато недостатъкът отразява обратното. Когато имате предимство или недостатък, хвърляте втори 20-стенен зар, когато правите хвърлянето. Ако имате предимство, използвайте по-високата от двете стойности, а ако имате недостатък, използвайте по-ниската стойност. Например, ако имате недостатък и хвърлите 17 и 5, използвате 5. Ако вместо това имате предимство и хвърлите тези числа, използвате 17.</p>
  </li>

  <li>Специфичното побеждава основното
  
  <br>
  <p>Много расови черти, класови характеристики, заклинания, магически предмети, способности на чудовища и други елементи на играта нарушават общите правила по някакъв начин, създавайки изключение за това как работи останалата част от играта. Ако <code>специфично правило</code> противоречи на <code>общо правило</code>, специфичното правило побеждава.</p>

  <blockquote>
  Изключенията от правилата често са малки. Например, мнозина приключенци не са компетентни в използването на дълги лъкове, но всеки елф от породата дървеник е, заради расовата му черта. Тази черта създава малко изключение в играта. Други примери за нарушаване на правилата са по-забележими. Например, един приключенец обикновено не може да преминава през стени, но някои заклинания го правят възможно. Магията обяснява повечето от големите изключения от правилата.
  </blockquote>
  <br>
  </li>

  <li>Закръгляне надолу
  
  <br>
  <p>Винаги когато разделяте число в играта, закръглете го надолу, ако получите дроб, дори ако дробът е половин или повече от половината.</p>
  </li>
</ul>
